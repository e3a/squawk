...
std::tuple<size_t, size_t> UpnpContentDirectoryFile::parseNode ( didl::DidlXmlWriter * didl_element, upnp::UpnpContentDirectoryRequest * request ) {
    ...
    for ( auto & item__ : item_list_ ) {
        ...
        { didl_element->container ( "/file/{}", "/file/{}", SquawkServer::instance()->dao()->object<didl::DidlContainer> ( item__.id() ) ); break; }

        case didl::objectContainerAlbumMusicAlbum:
        { didl_element->container ( "/file/{}", "/file/{}", SquawkServer::instance()->dao()->object<didl::DidlContainerAlbum> ( item__.id() ) ); break; }

        case didl::objectContainerAlbumPhotoAlbum:
        { didl_element->container ( "/file/{}", "/file/{}", SquawkServer::instance()->dao()->object<didl::DidlContainerPhotoAlbum> ( item__.id() ) ); break; }

        case didl::objectItem:
        {didl_element->write ( "/file/{}", "/file/{}", "resource/{0}.{1}", SquawkServer::instance()->dao()->object<didl::DidlItem> ( item__.id() ) ); break; }

        case didl::objectItemImageItemPhoto:
        { didl_element->write ( "/file/{}", "/file/{}", http_uri ( "resource/{0}.{1}" ), SquawkServer::instance()->dao()->object<didl::DidlPhoto> ( item__.id() ) ); break; }

        case didl::objectItemAudioItemMusicTrack:
        {didl_element->write ( "/file/{}", "/file/{}", http_uri ( "resource/{0}.{1}" ), SquawkServer::instance()->dao()->object<didl::DidlMusicTrack> ( item__.id() ) ); break; }

        case didl::objectItemVideoItemMovie:
        { didl_element->write ( "/file/{}", "/file/{}", http_uri ( "resource/{0}.{1}" ), SquawkServer::instance()->dao()->object<didl::DidlMovie> ( item__.id() ) ); break; }
