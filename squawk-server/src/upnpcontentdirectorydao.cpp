        ...
        try {
            ...
            while( stmt_type->step() ) {
                ...
                switch( stmt_type->get_int(0) ) {
                    case didl::objectContainer: type_ = "FOLDER"; break;
                    case didl::objectContainerAlbumMusicAlbum: type_ = "ALBUM"; continue;
                    case didl::objectContainerPersonMusicArtist: type_ = "ARTIST"; continue;
                    case didl::objectItem: type_ = "ITEM"; break;
                    case didl::objectItemAudioItemMusicTrack: type_ = "AUDIO_TRACK"; break;
                    case didl::objectItemImageItemPhoto: type_ = "IMAGE"; break;
                    case didl::objectItemVideoItemMovie: type_ = "MOVIE"; break;
                }
        ...
        try {
            ...
            while( stmt_artists_->step() ) {
                db::db_statement_ptr stmt_albums_artists_ = _db->prepareStatement (
                    "select count(*) from tbl_cds_object where artist = ? or publisher=?" );
                stmt_albums_artists_->bind_text(1, stmt_artists_->get_string( 2 ) );
                int albums_count_ = 0;
                if( stmt_albums_artists_->step() ) {
                    albums_count_ = stmt_albums_artists_->get_int( 1 );
                }
