...
std::ostream& operator<<(std::ostream& out, const parser_type & type) {
    ...
    case parser_type::REQUEST_URI: out << "REQUEST_URI"; break;
    case parser_type::REQUEST_PROTOCOL: out << "REQUEST_PROTOCOL"; break;
    case parser_type::REQUEST_VERSION_MAJOR: out << "REQUEST_VERSION_MAJOR"; break;
    case parser_type::REQUEST_VERSION_MINOR: out << "REQUEST_VERSION_MINOR"; break;
    case parser_type::REQUEST_KEY: out << "REQUEST_KEY"; break;
    case parser_type::REQUEST_VALUE: out << "REQUEST_VALUE"; break;
    case parser_type::REQUEST_BODY: out << "REQUEST_BODY"; break;
        ...
        if( type != parser_type::REQUEST_BODY && input[i] == '\r' ) {
            ...
            if( break_type == CR ) {
                if( type == parser_type::REQUEST_VERSION_MINOR) {
                    ss_buffer >> request.http_version_minor;
            ...
            case parser_type::METHOD: {
                if(input[i] == ' ' ) {
                    request.request_method = ss_buffer.str();
                    ss_buffer.str(std::string());
                    ss_buffer.clear();
                    type = http::HttpParser::next(type); //TODO ++
                } else {
